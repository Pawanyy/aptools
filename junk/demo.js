import QRCodeReader from "qrcode-reader";
import QrCode from "qrcode-reader";
// import base64QRString from "./data";
import Jimp from "jimp";
const base64QRString =
  "data:image/png;base64,"; // Replace this with your Base64 image string

async function decodeQRFromBase64(base64QRString, callback) {
  try {
    console.log("Base64 string:", base64QRString);

    const binaryData = Buffer.from(base64QRString.split(",")[1], "base64");

    const qrReader = new QRCodeReader();
    const image = await Jimp.read(binaryData);
    // Create an image buffer with width and height information
    const imgData = {
      data: binaryData,
      width: image.getWidth(),
      height: image.getHeight(),
    };

    console.log("imgData", imgData);

    // qrReader.decode(imgData, function (error, result) {
    //   if (error) {
    //     console.error("Error decoding QR code:", error.message);
    //     callback(null);
    //   } else {
    //     callback(result.data);
    //   }
    // });

    const result = await qrReader.decode(imgData);
    console.log(result);
    callback(result);
  } catch (err) {
    console.error("Error processing QR code:", err.message);
    callback(null);
  }
}

// Example usage
await decodeQRFromBase64(base64QRString, function (qrData) {
  if (qrData) {
    console.log("Decoded QR code:", qrData);
  } else {
    console.log("Failed to decode QR code.");
  }
});
