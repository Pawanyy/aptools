...
router.post("/generate-totp", async (req, res) => {
  try {
    ...
    if (base64_qr_image.trim() === "") {
      return res
        .status(400)
        .json({ message: "base64_qr_image can't be empty." });
    }
    ...
    decodeQRFromBinaryData(binaryData)
      .then((qrCodeText) => {
        ...
        if (!secret) {
          return res
            .status(400)
            .json({ message: "Invalid QR code. 2FA secret not found." });
        }
      ...
      .catch((error) => {
        ...
        res
          .status(500)
          .json({ message: "Failed to decode QR code.", error: error.message });
      });
  } catch (error) {
    console.error("Error generating TOTP code:", error);
    res
      .status(500)
      .json({ message: "Error generating TOTP code.", details: error.message });
  }
});
